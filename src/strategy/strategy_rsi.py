"""
RSI Strategy - Using Centralized Data
======================================
Runs RSI-based trading strategy using data from data/ folder.
...
2. Run: python strategy_rsi.py (run strategy instantly!)
...
rsi_threshold = 40  # RSI threshold for oversold condition
rsi_period = 14     # RSI calculation period
ma_period = 50      # Moving average period for trend filter

# Risk Management Settings (IMPROVED: Tighter stop-loss for mean-reversion)
stop_loss_pct = 0.04  # 4% stop-loss (tighter for RSI mean-reversion)
take_profit_pct = 0.15  # 15% take-profit (increased for better risk-reward)
use_trailing_stop = True  # Enable trailing stop-loss
trailing_stop_activation = 0.08  # Activate trailing stop at 8% profit
trailing_stop_distance = 0.04  # Trail 4% below peak
...
# Data Configuration
start_date = "2025-01-01"  # Data date range to load
end_date = "2025-12-31"
...
def calculate_profit_for_holding_period(data, holding_period):
    """Backtest the RSI-based trading strategy for a specific holding period with trailing stop"""
    ...
    for i in range(len(data) - holding_period):
        ...
        ###################################################################
        # ENTRY LOGIC (with improved filters)
        ###################################################################
        if not in_position:
            # Basic RSI condition
...
def test_stock(ticker, name, data):
    """Test RSI strategy on a single stock"""

    try:
        if data.empty or len(data) < max(rsi_period, ma_period) + 20:
        ...
        for holding_period in range(1, 31):
            final_investment, profit_loss_list = calculate_profit_for_holding_period(data, holding_period)
...
def main():
    """Run RSI strategy on data from data/ folder"""

    print("=" * 80)
    print("RSI STRATEGY - USING CENTRALIZED DATA")
    print("=" * 80)
    ...
    if summary_results:
        ...
        summary_filename = os.path.join(result_folder, f'rsi_strategy_results_{timestamp}.csv')
        ...
        if all_trades:
            ...
            trades_filename = os.path.join(result_folder, f'rsi_strategy_trades_{timestamp}.csv')
        ...
        if all_holding_period_results:
            ...
            holding_filename = os.path.join(result_folder, f'rsi_strategy_holding_analysis_{timestamp}.csv')

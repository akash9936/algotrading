...
2. Run: python strategy_rsi_sequential_MultipleTrade.py (run strategy!)
"""
...
take_profit_pct = 0.10   # 10% take-profit (increased for better risk/reward)
...
max_hold_days = 30       # Maximum days to hold a position (NEW)
cooldown_days = 10       # Days to wait before re-entering a losing stock (NEW)

# Transaction Cost Settings
transaction_cost = 0.001  # 0.1% transaction cost per trade

# Filter Settings (NEW)
use_trend_filter = False    # Only trade stocks above 50-day MA
...
def run_sequential_strategy(all_stock_data, stock_names):
    ...
    for ticker, data in all_stock_data.items():
        ...
        # NEW: Calculate trend filter (MA)
        data['MA'] = calculate_sma(data['Close'], ma_trend_period)

        # NEW: Calculate volume indicators
        if 'Volume' in data.columns:
            data['Volume_MA'] = calculate_volume_ma(data['Volume'], volume_ma_period)
    ...
    stock_cooldown = {}  # NEW: Track cooldown periods for losing stocks

    all_trades = []
    daily_portfolio_value = []
    missed_opportunities = []  # NEW: Track missed trading opportunities
    ...
    for date_idx, current_date in enumerate(all_dates):
        ...
        if in_position:
            ...
            if should_exit:
                ...
                if profit_loss < 0:
                    ...
                    stock_cooldown[ticker] = cooldown_end
                    print(f"           >>> Cooldown activated for {ticker} until {cooldown_end.date()}")
        ...
        ###################################################################
            ...
            for i in range(len(stock_list_sorted)):
                ...
                # NEW: Check cooldown period for this stock
                if ticker in stock_cooldown:
                    if current_date < stock_cooldown[ticker]:
                ...
                if rsi < rsi_threshold:
                    # NEW: Apply filters before entry
                    signal_valid = True

                    # Filter 1: Trend filter - price above MA
                    if use_trend_filter and 'MA' in stock_data.columns:
                        current_ma = stock_data.loc[current_date, 'MA']
                        current_price = stock_data.loc[current_date, 'Close']
                        if pd.isna(current_ma) or current_price < current_ma:
                            signal_valid = False

                    # Filter 2: Volume confirmation
                    if use_volume_filter and 'Volume' in stock_data.columns and 'Volume_MA' in stock_data.columns:
                        current_volume = stock_data.loc[current_date, 'Volume']
                        avg_volume = stock_data.loc[current_date, 'Volume_MA']
                        if pd.isna(avg_volume) or current_volume < avg_volume * volume_multiplier:
                            signal_valid = False

                    if not signal_valid:
                        # Record as missed opportunity due to filters
                        missed_opportunities.append({
                            'Date': current_date,
                            'Ticker': ticker,
                            'RSI': rsi,
                            'Reason': 'Filtered out (trend/volume)'
                        })
                        continue
    ...
    return capital, all_trades, daily_portfolio_value, missed_opportunities
...
def main():
    ...
    print("=" * 80)
    print("SEQUENTIAL RSI STRATEGY - ONE POSITION AT A TIME (IMPROVED v2.0)")
    print("=" * 80)
    print(f"Initial Investment: ₹{initial_investment:,.2f}")
    print(f"RSI Threshold: {rsi_threshold} | RSI Period: {rsi_period}")
    print(f"Stop Loss: {stop_loss_pct * 100}% | Take Profit: {take_profit_pct * 100}%")
    print(f"Max Hold: {max_hold_days} days | Cooldown: {cooldown_days} days")
    print(f"Trend Filter: {use_trend_filter} | Volume Filter: {use_volume_filter}")
    if use_date_filter:
    ...
    final_capital, all_trades, daily_portfolio, missed_opportunities = run_sequential_strategy(
    ...
    if all_trades:
        ...
        print(f"Missed Opportunities: {len(missed_opportunities)}")
